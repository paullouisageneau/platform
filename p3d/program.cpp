...
Program::Program(void)
{
	...
	if(!mProgram) throw Exception("Unable to create shader program");
}
...
void Program::link(void)
{
	...
	if(status != GL_TRUE)
	{
		...
		throw Exception(String("Unable to link shader: \n")+String(log));
	}
}
...
bool Program::hasVertexAttrib(const String &name)
{
...
int Program::getAttribLocation(const String &name)
{
	...
	if(it != mAttribLocations.end()) return it->second;
